...
rec {
  ...
  fill =
    value: xs:
    lib.foldl' (acc: x: acc // lib.setAttrByPath (if builtins.isList x then x else [ x ]) value) { } xs;
  ...
  mkModuleWithOptions =
    ...
    in
    {
      ...
      config = lib.mkIf (lib.getAttrFromPath enableOptionPath config && extraCondition) moduleConfig;
    };
